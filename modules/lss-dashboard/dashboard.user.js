...
function loadDashboard() {
    var prefix = lss_config.prefix +'_db';
    $('#'+prefix).html('<span class="glyphicon glyphicon-refresh spinning"></span> '+ I18n.t('lssm.dashboard.loading') +' ...');
    setTimeout(function() {
        $.get(lss_config.server+'/modules/lss-dashboard/dashboard.html',function(data) {
            $('#'+prefix).html(data);
        })
            .done(function(){
                loadGraphs();
...
(function ($, I18n) {
    ...
    $('#'+lss_config.prefix+'_menu').append(dasboard_button);
