((I18n, $) => {
    ...
    // Prepare hint area
    const fms7target = $(
        '<div id="fms7target" style="display:none; z-index: 999999; max-width: 260px; padding: 8px; background-color:#f58558">'
    );
    ...
    $('#building_list').on(
        ...
        function () {
            ...
            if (vehicleId && vehicleStates[vehicleId]) {
                fms7target.html(
                    'Unterwegs zu ' + vehicleStates[vehicleId].caption
                );
    ...
    radioMessage = data => {
        try {
            ...
            console.log(
                'There was an error processing the radioMessage() custom function: ' +
                    e
            );
        } finally {
                ...
                for (let match of res) {
                    if (match.indexOf('"fms_real":7') > 0) {
                        ...
                        if (dataObject.user_id === user_id) {
                            ...
                            vehicleStates[dataObject.id] = findNearest(
                                building_lat,
                                building_long,
                                targetType
                            );
