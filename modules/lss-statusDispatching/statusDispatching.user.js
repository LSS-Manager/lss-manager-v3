(function() {
	...
	function statusDispatching() {
		...
		if (getSetting('fastS5')) {
			...
			if (fms.length > 0) {
				if (fms[0].innerText
						.match(I18n.t('lssm.statusDispatching.fms')))
					fms[0].click();
			}
		}
		...
		for (var i = 0; i < nextVehicle.length; i++) {
			if (nextVehicle.length > 0
					&& nextVehicle[i].innerText.match(I18n
							.t('lssm.statusDispatching.nextVehicle'))) {
				nextVehicle[i].click();
				next_clicked = true;
			} else {
				count++;
		...
		for (var i = 0; i < label.length; i++) {
			if (label[i].innerText.match(I18n
					.t('lssm.statusDispatching.fms5H1'))) {
				allTransported = true;
			}
		}
