...
(function ($, I18n) {
    ...
    I18n.translations.it_IT['lssm']['aaozaehler'] = {
        ...
        description1:
            'Confina un AAO una volta che è stato cliccato una volta.',
    };
    ...
    I18n.translations.nl_NL['lssm']['aaozaehler'] = {
        //AUR
    ...
    const getSetting = setting =>
        lssm.managedSettings.getSetting(SETTINGS, setting);
    ...
    function erstelleZaehler() {
        $('.aao, .vehicle_group').each(function () {
            $(this)
                .find('span:not(.glyphicon):first')
                .after(' <span class="aaoZaehler">0</span>x');
        });

        // Zähler-Reset Button
        $('#mission-aao-group').after(
            `<button id="resetAAOZaehler" class="btn  btn-default btn-xs" type="button">${I18n.t(
                'lssm.aaozaehler.reset'
            )}</button>`
        );
