(I18n => {
    ...
    I18n.translations.de_DE.lssm.destfilter = {
        ...
        settings: {
            ...
            cellsYellow:
                'Polizeistationen/Zellen mit weniger freien Zellen als benötigt ausblenden',
    ...
    I18n.translations.en_US.lssm.destfilter = {
        ...
        settings: {
            ...
            cellsYellow:
                'Hide police stations/cells with fewer free cells than required',
    ...
    I18n.translations.pt_PT.lssm.destfilter = {
        ...
        settings: {
            ...
            cellsYellow:
                'Esconder esquadras/células com menos células livres do que o necessário',
        },
    };
    ...
    I18n.translations.sv_SE.lssm.destfilter = {
        ...
        settings: {
            ...
            cellsYellow:
                'Dölj polisstationer/celler med färre lediga celler än vad som krävs',
        },
    };
    I18n.translations.da_DK.lssm.destfilter = {
        ...
        settings: {
            ...
            cellsYellow:
                'Skjul politistationer/celler med færre ledige celler end påkrævet',
        },
    };
    I18n.translations.nb_NO.lssm.destfilter = {
        ...
        settings: {
            ...
            cells: 'Skjul hele celler',
            cellsYellow:
                'Skjul politistasjoner/celler med færre frie celler enn nødvendig',
        },
    };
    I18n.translations.it_IT.lssm.destfilter = {
        ...
        settings: {
            ...
            distance:
                'Nascondi destinazioni oltre la distanza di x km (0 disattiva)',
            tax: 'Nascondi destinazioni con una TASSA superiore a x%',
            cells: 'Nascondi celle piene',
            cellsYellow:
                'Nascondere le stazioni di polizia/celle con meno celle libere del necessario',
        },
    };
    I18n.translations.fr_FR.lssm.destfilter = {
        ...
        settings: {
            ...
            distance:
                'Cacher les destinations situées à plus de x km de distance (0 désactive)',
            tax: 'Cacher les destinations dont la TAXE est supérieure à x%',
            cells: 'Masquer les cellules pleines',
            cellsYellow:
                'Cachez les commissariats de police / cellules avec moins de cellules libres que nécessaire',
        },
    };
    ...
    I18n.translations.ro_RO.lssm.destfilter = {
        ...
        settings: {
            ...
            distance:
                'Ascundedestinațiile peste x km distanță (0 dezactivează)',
            tax: 'Ascundeți destinațiile cu o TAXĂ mai mare de x%',
            cells: 'Ascunderea celulelor complete',
            cellsYellow:
                'Ascunderea secțiilor/celulelor de poliție cu mai puține celule libere decât este necesar',
        },
    };
    I18n.translations.nl_NL.lssm.destfilter = {
        ...
        settings: {
            ...
            distance:
                'Verberg bestemmingen boven x km afstand (0 wordt gedeactiveerd)',
            tax: 'Verberg bestemmingen met een Kosten hoger dan x%.',
            cells: 'Verberg volle cellen',
            cellsYellow:
                'Verberg politiebureaus/cellencomplexen met minder vrije cellen dan nodig zijn',
    ...
    const mode = document.querySelector('a.btn[href*="/patient/"]')
        ? 'hospital'
        : 'prison';

    const getSetting = key =>
        lssm.managedSettings.getSetting(LSS_DESTFILTER_STORAGE, key);
    const filter = () => {
        if (mode === 'hospital') {
            let counter = 1;
            document
                .querySelectorAll('.col-md-9 tbody > tr .visible-xs')
                .forEach(el => {
                    const hide = () =>
                    ...
                    let freeBeds = parseInt(
                        info
                            .match(
                                new RegExp(
                                    `${I18n.t(
                                        'lssm.destfilter.freeBeds'
                                    )}:\\W+-?\\d+`
                                )
                            )[0]
                            .replace(/[^-0-9]*/g, '')
                    ...
                    if (
                        getSetting('distance') > 0 &&
                        distance > getSetting('distance')
                    )
                        return hide();
                    if (getSetting('beds') && freeBeds === 0) return hide();
                    if (
                        getSetting('beds1') > 0 &&
                        freeBeds < getSetting('beds1')
                    )
                        return hide();
                    if (getSetting('department') && !department) return hide();
                    getSetting('firstOwn') > 0 &&
                        !tax &&
                        counter > getSetting('firstOwn') &&
                        hide();
            ...
            document
                .querySelectorAll('.alert-info a.btn[href*="/gefangener/"]')
                .forEach(el => {
                    ...
                    let tax = 0;
                    let taxMatch = el.innerText.match(
                        new RegExp(
                            `${I18n.t('lssm.destfilter.cellTax')}:\\W+\\d+%`
                        )
                    );
                    if (taxMatch)
                        tax = parseInt(taxMatch[0].replace(/\D+/g, ''));
                    getSetting('cells') &&
                        ...
                        hide();
                    getSetting('cellsYellow') &&
                        el.classList.contains('btn-warning') &&
                        hide();
                    if (taxMatch)
                        tax = parseInt(taxMatch[0].replace(/\D+/g, ''));
                    if (getSetting('distance') > 0)
                        distance > getSetting('distance') && hide();
                    tax > getSetting('tax') && hide();
                });
        }
    };
    const settingNode = (key, type, attributes) => {
        ...
        inputNode.addEventListener('change', e => {
            managedSettings.settings[key].value =
                type === 'checkbox'
        ...
        wrapper.insertAdjacentText(
            'beforeend',
            I18n.t(`lssm.destfilter.settings.${key}`)
        );
        return wrapper;
    };
    ...
    mode === 'hospital' &&
        settingsNode.appendChild(settingNode('beds', 'checkbox'));
    mode === 'hospital' &&
        settingsNode.appendChild(settingNode('department', 'checkbox'));
    mode === 'prison' &&
        settingsNode.appendChild(settingNode('cells', 'checkbox'));
    mode === 'prison' &&
        gefkwCountries.includes(I18n.locale) &&
        settingsNode.appendChild(settingNode('cellsYellow', 'checkbox'));
    settingsNode.appendChild(
        settingNode('distance', 'number', {
            min: 0,
        })
    );
    settingsNode.appendChild(
        settingNode('tax', 'number', {
            min: 0,
            max: 50,
            step: 10,
        })
    ...
    mode === 'hospital' &&
        settingsNode.appendChild(
            settingNode('firstOwn', 'number', {
                min: 0,
            })
        );
    mode === 'hospital' &&
        settingsNode.appendChild(
            settingNode('beds1', 'number', {
                min: 0,
                max: 30,
            })
        );
    ...
    document
        ...
        .insertAdjacentElement('afterend', settingsNode);
    document
        .querySelector('.alert-info')
        .insertAdjacentElement('afterend', settingViewButton);
    getSetting('seeInfos')
        ? settingsNode.classList.remove('hidden')
        : settingsNode.classList.add('hidden');
    filter();
