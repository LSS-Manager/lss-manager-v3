(function (I18n, $) {
    ...
    function replaceMissingText(text) {
        if (text !== null && text !== '') {
            text = text.replace(
                'Ein Fahrzeug hat einen Sprechwunsch!',
                'Hallo i bims. 1 Sprechwumsch.'
            );
            text = text.replace('Ein', '1');
            text = text.replace('ein', '1');
            text = text.replace(
                'Zusätzlich benötigte Fahrzeuge',
                'Wir brauchem weitere 1satzfahrzeuge'
            );
        }
        return text;
    }
    ...
    function replacePatientMarker(text) {
        if (text !== null && text !== '') {
            text = text.replace(
                'Wir benötigen ein NEF.',
                'Wir brauchen 1 NEF vong Dringlichkeit her.'
            );
            text = text.replace(
                'Wir benötigen einen RTW.',
                'lol. Chique mal 1 RTW.'
            );
        }
        return text;
    }
    ...
    function replaceMissionCaption(text) {
        if (text !== undefined && text !== '') {
            ...
            text = text.replace(
                ...
                'Fassadenteile weak vong Festigkeit her'
            );
