(function (I18n, $) {
    ...
    let prefix = 'saveVGE',
        saveMissions = JSON.parse(localStorage.getItem(prefix)) || {},
        isCreate = false;
    function readData() {
        let daten = $('#new_mission_position').serializeArray(),
            notSave = [
                'utf8',
                'authenticity_token',
    ...
    function buildOptions(onlyOption) {
        let select = $(
            '<ul class="dropdown-menu" aria-labelledby="vgeSaveDropdown" id="vgeSaveDropdownUl"></ul>'
        );
        let h = '';
    ...
    function saveData(e) {
        e.preventDefault();
        let d = readData();
        let savedItem = $('#mission_position_mission_custom_caption')
            .val()
            .trim();
    ...
    function checkNoVGEs() {
        if ($('#vgeSaveDropdownUl').has('li').length === 0) {
            $('#vgeSaveDropdownUl').html(
                '<li>' + I18n.t('lssm.saveVGE.noVGE') + '</li>'
            );
        }
    }
    function delData(e) {
    ...
    function createSettings() {
        let div = $(
            '<div class="dropdown" id="' +
                prefix +
                '"><button class="btn btn-default dropdown-toggle" type="button" id="vgeSaveDropdown" name="vgeSaveDropdown" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true">' +
                I18n.t('lssm.saveVGE.VGEs') +
        ...
        let s = $(
            '<a href="#" class="btn btn-sm btn-success">' +
                I18n.t('lssm.saveVGE.save') +
                '</a>'
        ).click(saveData);
        let d = $(
            '<a href="#" class="btn btn-sm btn-danger">' +
                I18n.t('lssm.saveVGE.del') +
                '</a>'
        ).click(delData);
    ...
    $('#buildings_outer').on('change', function () {
        if (
            $('#mission_position_mission_type_id_-1').prop('checked') &&
            !isCreate
        ) {
            createSettings();
            isCreate = true;
