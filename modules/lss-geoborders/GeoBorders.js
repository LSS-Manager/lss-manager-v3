(($, I18n) => {
    ...
    lssm.loadStyle('/modules/lss-geoborders/vendor/jquery.tree-multiselect.min.css');
    ...
    const renderSettings = (data, selected) => {
        ...
        let markup = '<div id="borders-modal" style="display: none; z-index: 99999; background: #fff; top: 20px; position: absolute; width: 50%; left: 25%" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">';
        markup += '<div class="modal-header">';
        markup += '<button type="button" class="close borders-close" data-dismiss="modal" aria-hidden="true">×</button>';
        ...
        markup += '<button class="btn borders-close" data-dismiss="modal" aria-hidden="true">Schließen</button>';
        markup += '<button id="borders-btn-save" class="btn btn-primary">Speichern</button>';
        markup += '</div>';
        markup += '</div>';
        ...
        $('#borders-btn-save').click(() => {
            localStorage.setItem(STORAGE, JSON.stringify($('#borders-selection').val()));
            location.reload();
        });

        $.getScript(lssm.getlink('/modules/lss-geoborders/vendor/jquery.tree-multiselect.min.js'), () => {
            $("#borders-selection").treeMultiselect({ searchable: true, startCollapsed: true });
    ...
    $.getJSON(lssm.getlink('/modules/lss-geoborders/geos/geo_de.json'), (data) => {
