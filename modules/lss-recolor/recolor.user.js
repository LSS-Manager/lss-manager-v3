(function ($, I18n) {
    function setColor(bgcolor, color) {
        ...
        $('head').append(
            "\
        <style type='text/css' rel='stylesheet' id='" +
                lssm.config.prefix +
                "_recolor_style'> \
            #dashboard_outer, .panel-heading, .modal-header, nav, .navbar-default {background-color:" +
                bgcolor +
                '!important; color:' +
                color +
                "!important }\
            .navbar-nav > li > a, a[id^='mission_caption']{color:" +
                color +
                '!important}\
        </style>'
        );
    ...
    let settingButton = $(
        '<li role="presentation"><a id="' +
            lssm.config.prefix +
            '_recolor_open" href="#">Recolor</a></li>'
    );
    ...
    $('#' + lssm.config.prefix + '_recolor_open').click(function () {
        ...
        let settings_html =
            '<div id="' +
            lssm.config.prefix +
            '_recolor"><div class="jumbotron">' +
            '<a href="#" class="close" data-dismiss="alert" aria-label="close" id="' +
            lssm.config.prefix +
            '_recolor_close">&times;</a>' +
            '<h1>Recolor</h1>' +
        ...
        settings_html +=
            ...
            '<li><a href="#" id="' +
            lssm.config.prefix +
            '_recolor_save" class="btn btn-success">Speichern</a></li></ul>' +
            '</div></nav>';
        ...
        settings_html +=
            ...
            '<input type="color" value="#c9302c" id="' +
            lssm.config.prefix +
            '_recolor_bg">' +
            '</div></div></div>';
        ...
        settings_html +=
            ...
            '</div>' +
            '<div class="panel-footer">' +
            '<input type="color" value="#ffffff" id="' +
            lssm.config.prefix +
            '_recolor_text">' +
            '</div></div></div>';
        ...
        $('#' + lssm.config.prefix + '_recolor_bg').change(function () {
            setColor(
                $(this).val(),
                $('#' + lssm.config.prefix + '_recolor_text').val()
            );
        });
        $('#' + lssm.config.prefix + '_recolor_text').change(function () {
            setColor(
                $('#' + lssm.config.prefix + '_recolor_bg').val(),
                $(this).val()
            );
        });
