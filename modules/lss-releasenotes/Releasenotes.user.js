(function (I18n, $) {
    ...
    function renderLayer() {
        ...
        markup +=
            'max-width: 600px; width: 80%; border: 1px solid rgb(66, 66, 66);">';
        markup +=
            '<button type="button" class="close" aria-label="Cool">×</button>';
        markup += '<div class="container-fluid">';
        markup += '<h4>' + I18n.t('lssm.releasenotes.title') + '</h4>';
        markup += '<div id="releaseNotesContent"></div>';
        markup +=
            '<button class="releaseNotesClose">' +
            I18n.t('lssm.releasenotes.close') +
            '</button>';
        markup += '</div>';
        markup += '</div>';
        ...
        $.get(lssm.getlink('/modules/lss-releasenotes/releaseNotes.json', 10)) // cache for 10 Minutes
            .fail(function () {
                $('#releaseNotesContent').html(
                    '<div>' +
                        I18n.t('lssm.releasenotes.errorloading') +
                        '</div>'
                );
            })
            .done(function (data) {
