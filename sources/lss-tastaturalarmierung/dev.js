...
html = html + '   </div>';
html =
    html + '   <div style="width: 100%; display: flex; padding-bottom: 6px">';
html =
    html +
    '      <input class="string form-control" id="search_vehicle" size="50" type="text">';
html =
    html +
    '      <a id="key_settings_button" href="#" class="btn btn-success navbar-btn btn-sm" style="width: 64px; margin: 0; margin-left: 6px" title="Einstellungen">';
html =
    html +
    '         <span class="glyphicon glyphicon-cog" style="font-size: 16px; margin-top: 2px"></span>';
html = html + '      </a>';
html = html + '   </div>';
html =
    html +
    '   <div id="key_settings" style="width: 100%; display: none; flex-direction: column; padding-top: 12px; padding-bottom: 12px">';
html =
    html +
    '      <div id="key_settings_console" style="width: 100%; min-height: 20px; margin-top: 12px; margin-bottom: 12px">';
html = html + '         <font></font>';
html = html + '      </div>';
html =
    html +
    '      <table id="key_settings_table" class="table table-striped tablesorter tablesorter-default">';
html = html + '         <tbody>';
...
html =
    html +
    '      <font style="font-size: 12px">by ChaosKai93 (build 2016-10-14-0054)</font><a href="https://github.com/ChaosKai/tastaturalarmierung" target="_blank" style="font-size: 12px; margin-left: 24px">GitHub Projekt</a>';
html = html + '   </div>';
html = html + '</div>';
...
$.each(keyList, function (key, value) {
    ...
    html =
        html +
        '  <td style="text-align: left"><font style="font-weight: 700" title="' +
        value.description +
        '">' +
        value.name +
        '</font></td>';
    html =
        html +
        '  <td style="text-align: left"><font id="' +
        key +
        '_key_code"></font></td>';
    html = html + '  <td style="text-align: right">';
    html =
        html +
        '    <a href="#" class="btn btn-success navbar-btn btn-sm" style="margin: 0" onclick="setKey(' +
        "'" +
        key +
        "'" +
        ')">Taste belegen</a>';
    html =
        html +
        '    <a href="#" class="btn btn-danger navbar-btn btn-sm" style="margin: 0" onclick="delKey(' +
        "'" +
        key +
        "'" +
        ')">Belegung löschen</a>';
...
$(
    (function () {
        ...
        $(document).on('keyup', function (event) {
            ...
            // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
            if (!set_key_mode) {
                let table = $('#vehicle_show_table_body_all').length
                    ? '#vehicle_show_table_body_all tr'
                    : '#vehicle_show_table_rett tr';
                ...
                if (event.which === getKeyStorage('fhz1')) {
                    $('#search_vehicle').val(
                        $('#search_vehicle')
                            .val()
                            .substring(0, $('#search_vehicle').val().length - 1)
                    );
                    updateTable();
                    $(table + ':visible')
                        .eq(0)
        ...
        $(document).on('keyup', function (event) {
            if (set_key_mode) {
                ...
                $('#key_settings_console')
                    .find('font')
                    .text('Die Taste wurde gespeichert!');
                window.setTimeout(function () {
                    $('#key_settings_console').find('font').text('');
                }, 1500);
    ...
    $(table).each(function () {
        if (
            $(this)
                ...
                .text()
                .toLowerCase()
                .indexOf($('#search_vehicle').val().toLowerCase()) >= 0
        ) {
            // Found vehicle name
...
function setKey(key) {
    ...
    $('#key_settings_console')
        .find('font')
        .text('Drücke die Taste, die Du für diese Funktion belegen möchtest..');
    $('#search_vehicle').focus();
}
...
function delKey(key) {
    ...
    $('#key_settings_console')
        .find('font')
        .text("Die Taste '" + key + "' wurde gelöscht!");
    window.setTimeout(function () {
        $('#key_settings_console').find('font').text('');
    }, 1500);
}
